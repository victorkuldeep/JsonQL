    ...
    for (const idx of indices) {
        ...
        if (spec.groupBy) {
            for (const field of spec.groupBy) {
                const v = (0, utils_js_1.getPath)(item, field) ?? null;
        ...
        for (const agg of spec.aggs) {
            ...
            switch (op) {
                case "COUNT":
                    ...
                    else if ((0, utils_js_1.getPath)(item, agg.field)) {
                        state.count++;
                    }
                    break;
                case "SUM":
                ...
                case "MAX": {
                    ...
                    if (!field)
                        break;
                    const v = (0, utils_js_1.getPath)(item, field);
                    if (typeof v === "number") {
                        state.sum += v;
...
function filterData(data, filter) {
    const query = (0, cache_js_1.parseQueryCached)(filter);
    const options = { caseSensitive: query.caseSensitive, strict: query.strict };
    const indices = [];
    for (let i = 0; i < data.length; i++) {
        if ((0, engine_js_1.evalExpr)(query.expr, data[i], options, i)) {
            indices.push(i);
        }
    }
    return indices;
...
function distinctIndices(data, indices, fields) {
    ...
    for (const idx of indices) {
        ...
        for (const f of fields) {
            const v = (0, utils_js_1.getPath)(item, f) ?? null;
            keyParts.push(JSON.stringify(v));
            obj[f] = v;
        }
        const key = keyParts.join("|");
